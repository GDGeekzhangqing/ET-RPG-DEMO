...
{
    void ActionHandle(ref BuffHandlerVar buffHandlerVar);
}

public interface IBuffActionWithSetOutputHandler
{
    IBufferValue[] ActionHandle(ref BuffHandlerVar buffHandlerVar);
}

public interface IBuffRemoveHanlder 
{
    void Remove(ref BuffHandlerVar buffHandlerVar);//正常的效果移除之类,会调用该方法
}

public interface IBuffUpdateHanlder
{
    void Update(ref BuffHandlerVar buffHandlerVar);//一些buff的效果更新/刷新. 用这个
}
